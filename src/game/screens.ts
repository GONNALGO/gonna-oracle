...
export function drawTitle(ctx: CanvasRenderingContext2D, t: number, art: Art): void {
  ...
  ctx.restore();
  drawText(ctx, 'V1.0 BYZANTINE', VW - textWidth('V1.0 BYZANTINE', 1) - 8, VH - 14, 1, '#5a5f6c');
  drawText(ctx, '(C) TAKATUSH & THE BYZANTINES', 8, VH - 14, 1, '#5a5f6c');
}
...
export function drawVictory(ctx: CanvasRenderingContext2D, stats: FinalStats, t: number): void {
  ...
  if (t > 60) {
    ...
    drawTextSh(ctx, 'TAKATUSH & THE BYZANTINES', VW / 2, 182, 1, '#8a8f9c', 'center');
  }
