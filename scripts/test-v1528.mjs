...
console.log('\n[8] share links: ?st= for verified single-mode cards only');
{
  ...
  // M-4: the live mode was renamed 'testnet' -> 'live'; links carry ?arena=live
  store.set('gonna.arena.adapter.testnet', 'live');
  ok(shareUrl(42, 4) === 'http://localhost/?arena=live&duel=42&st=4', 'live link carries &st=4');
  ok(shareUrl(42, null) === 'http://localhost/?arena=live&duel=42', 'no stage -> no hint');
  ok(shareUrl(42, 9) === 'http://localhost/?arena=live&duel=42', 'out-of-range stage -> no hint');
  store.set('gonna.arena.adapter.testnet', 'mock');
