#!/usr/bin/env python3
"""QuantumArena v3 TESTNET smoke — app 770816020.
...
MBR = 358_200
OPUP = 769688641  # testnet opcode-budget donor app
...
players = []
need = 3
state_extra = os.path.join(HERE, 'testnet-v3-smoke.json')
...
call = mcall(PB, 'resolve(uint64,uint64,byte[],byte[])byte[]', [cid, 0, b'', vsig],
             accounts=[PA.addr, json.load(open(os.path.join(HERE, 'testnet.json')))['treasury_addr']],
...
roster = [PA, PB, players[0], players[1], players[2]]
before = {w.addr: gonna_balance(w) for w in roster}
cid = create_challenge(PA, 4, 0, 7000)
...
claimers = [players[2], PA, players[0], PB, players[1]]
for seat in range(5):
...
print(f'T2 OK: all 5 refunded in full, zero fee, boxes deleted, MBR back')
