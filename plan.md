...
- 2026-08-23 — ARENA v12 CONTINUE (decisioni Prince, definitive): 5 ALGO flat al TREASURY, MAX 1 continue per giocatore per match, retry su STESSO seed ("stesso campo stessa palla" — seed nuovo cambierebbe il soffitto punti), best-of-2 automatico (il sigillato è il MIGLIORE dei due run, mai peggiorarsi). Enforcement oracle lato client (testnet): 2ª firma punteggio richiede receipt on-chain `QA-CONTINUE|<draftId|challengeId>|<addr>` (pay 5 ALGO esatti → treasury, note exact match, txid single-use in localStorage — mainnet = server oracle con DB). Contratto ZERO modifiche: la chain vede solo il punteggio finale. Random seed riservato alla creazione card in v2 mainnet.
